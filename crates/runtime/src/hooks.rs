...
mod tests {
    ...
    #[test]
    fn supports_post_tool_use_failure_hooks() {
        let runner = HookRunner::new(RuntimeHookConfig::new(
            Vec::new(),
            Vec::new(),
            vec![shell_snippet("printf 'tool failed cleanup'")],
        ));

        let result = runner.run_post_tool_use_failure("bash", r#"{"command":"pwd"}"#, "boom");

        assert_eq!(
            result,
            HookRunResult::allow(vec!["tool failed cleanup".to_string()])
        );
    }

    #[test]
    fn supports_abort_signal_and_progress_reporting() {
        let runner = HookRunner::new(RuntimeHookConfig::new(
